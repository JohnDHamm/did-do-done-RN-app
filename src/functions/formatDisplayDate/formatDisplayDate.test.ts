...
describe('formatDisplayDate()', () => {
  it('returns formatted date for display', () => {
    ...
    expect(formatDisplayDate({ date: date1, showDay: false })).toEqual(
      formattedDate1
    );
    // (different year) => 'Nov 3, 2019'
    const date2 = new Date('11-03-2019').getTime();
    expect(formatDisplayDate({ date: date2, showDay: false })).toEqual(
      "Nov 3, '19"
    );
    // (this week) => 'Thursday, Jul 9'
    const date3 = new Date('07-09-2020').getTime();
    expect(formatDisplayDate({ date: date3, showDay: true })).toEqual(
      'Thursday, Jul 9'
    );
  });
});
